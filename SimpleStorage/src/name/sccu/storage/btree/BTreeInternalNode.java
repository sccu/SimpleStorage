...
class BTreeInternalNode implements BTreePage {
	...
	BTreeInternalNode() throws IOException {
	...
	BTreeInternalNode(int pageNumber) {
	...
	/*
	 * private void validate() throws IOException { for (int i = 0; i <
	 * this.keyCount - 1; i++) { if (!this.getKey(i).lessThan(this.getKey(i+1)))
	...
	public BTreeKey splitLeaf(BTreeRecord record, BTreePage rightPage, int index)
			throws IOException {
		throw new UnsupportedOperationException();
	}

	public BTreeKey splitNode(BTreeKey key, int newChild, BTreePage rightPage,
			int index) throws IOException {
		BTreeInternalNode tempPage = new BTreeInternalNode(-2);
		...
		tempPage.copyNode(rightPage, midIndex + 1, tempPage.keyCount - midIndex
				- 1);
	...
	public void merge(BTreePage sibling, BTreePage parent, StackItem item) throws IOException {
	...
	public void redistribute(BTreePage siblingPage, BTreePage parent, int index)
		...
		if (child.getKey(0).lessThan(sibling.getKey(0))) {
			...
			for (int i = 0; i < moveCount - 1; i++) {
				...
				child.setChild(child.getKeyCount() + i + 2,
						sibling.getChild(i + 1));
			}
			...
			for (int i = 0; i < moveCount - 1; i++) {
				...
				child.setChild(i + 1,
						sibling.getChild(sibling.getKeyCount() + i + 2));
			}
